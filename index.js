...
discord.once('ready', () => {
    console.log('online');
    discord.user.setActivity('THE WAAAAAAAALLLLLL', { type: "WATCHING"} )
});
...
//            const comando = await discord.guilds.cache.get(config.guild)?.commands.fetch([config.moderation.ban, config.moderation.kick, config.moderation.unban])
...
discord.on("messageCreate", (cmd) => {
    if (cmd.content === 'xy.audio') {
        let voice = cmd.member.voice.channel;

        if(!voice) {
            cmd.channel.send('no voice, no song')
        } else {
            voice.join()
                .then(voice2 => {
                    voice2.play('hazard.mp3')
                })
        }
    }
})
